...
def display_header(win):
    ...
    lbl_rep_status.grid(row=0, column=6)


def edit_rep(obj, window):
    window.destroy()
    # TODO: Complete this
...
def print_rep(obj):
    pass
    # TODO: Complete this, do this first using .xlsx or .docx
...
def viewall():
    ...
    for j, i in enumerate(object_list):
        ...
        btn_edit_report = Button(window, text="Edit Report", width=12, command=lambda: edit_rep(i, window))
        btn_print_rep = Button(window, text="Print Report", width=12, command=lambda: print_rep(i))
        ...
        lbl_rep_status.grid(row=j + 1, column=6)
        btn_edit_report.grid(row=j + 1, column=7)
        btn_print_rep.grid(row=j + 1, column=8)
